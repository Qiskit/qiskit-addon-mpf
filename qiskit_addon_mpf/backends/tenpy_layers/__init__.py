...
"""A layer-wise time-evolution backend using :external:mod:`tenpy`.

.. currentmodule:: qiskit_addon_mpf.backends.tenpy_layers
...
:mod:`~qiskit_addon_mpf.backends.tenpy_tebd` module. The main difference is that this module gives
the user full flexibility for defining their product formulas, thereby not limiting them to the
options built into the :external:mod:`tenpy` library.
...
also :external:meth:`~tenpy.algorithms.tebd.TEBDEngine.evolve_step`) this implementation outsources the
responsibility of updating bonds in alternating fashion to the definition of multiple time-evolution
**layers**.
...
It is not possible to instruct TeNPy's TEBD algorithm to simulate the exact structure of the circuit
...
structure other than the ones implemented in TeNPy (like the example above), then one can use the
time-evolution algorithm provided by this module. Rather than taking a single monolithic Hamiltonian
whose time-evolution is to be modeled, the :class:`~.tenpy_layers.LayerwiseEvolver` accepts a list
of :class:`~.tenpy_layers.LayerModel` objects, each one describing an individual layer of the
...
use it to construct a :class:`~.tenpy_layers.LayerwiseEvolver` which can be used to replace the
:class:`.tenpy_tebd.TEBDEvolver` in the workflow described in :mod:`.tenpy_tebd`.

.. hint::
   The overall workflow of using this module is the same as of the :mod:`.tenpy_tebd` module, so be
   sure to read those instructions as well.

Simply put, we must convert each one of the circuit ``layers`` (see above) into a
:class:`~.tenpy_layers.LayerModel` instance. For this purpose, we can use its
:meth:`~.tenpy_layers.LayerModel.from_quantum_circuit` method.
...
that the TEBD algorithm underlying the :class:`~.tenpy_layers.LayerwiseEvolver` must update even and
...
In the end, we have 8 :class:`~.tenpy_layers.LayerModel`'s, one for each of the 4 two-qubit layers,
...
mix-and-match a :class:`.tenpy_layers.LayerwiseEvolver` with a :class:`.tenpy_tebd.TEBDEvolver`.
