...
from qiskit_addon_mpf.static.exact import setup_exact_model
from qiskit_addon_mpf.static.lse import setup_lse


@ddt
class TestExactCoeffs(unittest.TestCase):
    """Tests for the ``qiskit_addon_mpf.static.exact`` module."""

    def test_setup_exact_model(self):
        """Tests the :meth:`.setup_exact_model` method."""
        trotter_steps = [1, 2, 4]
        ...
        with self.subTest("final cost"):
            ...
            self.assertAlmostEqual(final_cost, 1.888888888888)
    ...
    def test_exact_order_1_references(
        self, trotter_steps: list[int], expected_coeffs: list[float], expected_cost: float
    ):
        ...
        with self.subTest("final cost"):
            final_cost = problem.solve()
            self.assertAlmostEqual(final_cost, expected_cost)
    ...
    def test_exact_higher_order_references(
        ...
        """
        lse = setup_lse(trotter_steps, order=order, symmetric=True)
        problem, coeffs = setup_exact_model(lse)
    ...
    def test_setup_exact_model_params(self):
        """Tests the :meth:`.setup_exact_model` method with parameters."""
        ks = cp.Parameter(2)
        lse = setup_lse(ks, order=1)
        problem, coeffs = setup_exact_model(lse)
